...
namespace MTNAutomationTests.TestCases.Master_Terminal.General.Ownership_Change_Enquiry
{
    ...
    public class TestCase66003 : MTNBase
    {
        const string MSLDebtor = "RDT";   // Operator MSL's Debtor
        const string CADebtor = "DBTCAINCOT";
        
        const string CargoId = "COO66003A0";
        const string COOId = "COO66003";
        ...
        void MTNInitialize()
        {
           ...
           new InvoiceTransactionMapForm()
            ...
            CallJadeScriptToRun(TestContext, "resetData_66003");
        }

        [TestMethod]
        public void INCOPastChargeNewPriorChargedOld()
        {
            MTNInitialize();

            var date = DateTime.Now;
            var effective = date.AddDays(-10);
            ...
            ownershipChangeMaintForm
                .AddCOO(new OwnershipChangeDetailsAddArguments
                {
                    COOFieldsToSet = new []
                    {
                        ...
                        { FieldRowName = OwnershipChangeMaintForm.ControlFieldName.COONumber, FieldRowValue = $"{COOId}D" },
                        new MTNGeneralArguments.FieldRowNameValueArguments
                    ...
                    CargoSearchArguments = new CargoSearchFormArguments
                    {
                            ...
                            {
                                new MTNGeneralArguments.FieldRowNameValueArguments
                                { FieldRowName = CargoSearchForm.ControlFieldName.Site, FieldRowValue = "On Ship" },
                                new MTNGeneralArguments.FieldRowNameValueArguments
                                { FieldRowName = CargoSearchForm.ControlFieldName.CargoID, FieldRowValue = $"{CargoId}" }
                            },
                            CargoRowsToAdd = new []
                            {
                ...
                .DischargeSelectedCargo(new VoyageOperationsFormArguments
                 {
                     ...
                     {
                         new MTNGeneralArguments.FieldRowNameValueArguments
                         { FieldRowName = VoyageOperationsForm.ControlFieldName.DischargeTo, FieldRowValue = "INCOCA" },
                         new MTNGeneralArguments.FieldRowNameValueArguments
                         { FieldRowName = VoyageOperationsForm.ControlFieldName.OpsRetrospective, FieldRowValue = CheckboxValue.Set },
                         new MTNGeneralArguments.FieldRowNameValueArguments
                         { FieldRowName = VoyageOperationsForm.ControlFieldName.EffectiveDateDate, FieldRowValue = effective.ToString("ddMMyyyy") },
                         new MTNGeneralArguments.FieldRowNameValueArguments
                         { FieldRowName = VoyageOperationsForm.ControlFieldName.EffectiveDateTime, FieldRowValue = effective.ToString("HHmm") },
                     }
                 }, new[] { $"ID^{CargoId}0", });
            ...
            cargoEnquiryTransactionForm.TblTransactions2.FindClickRow(new FindClickRowArguments
                { DetailsToSearchFor = new [] { "Type^Edited", "Type^Curtain Attached" }});
            ...
            cargoEnquiryTransactionForm.INCODebtorChargedValidation(effective,
                new[] { $"Debtor^{CADebtor}~Transaction^Edited~Narration^ILTINCOT",
                    $"Debtor^{MSLDebtor}~Transaction^Curtain Attached~Narration^ILTINCOT"
                }, new[] { CADebtor, MSLDebtor });
