...
namespace MTNAutomationTests.TestCases.Master_Terminal.Gate_Functions.Prenotes
{
    ...
    public class TestCase43771 : MTNBase
    {
        ...
        private const string TestCaseNumber = @"43771";
        ...
        void MTNInitialize()
        {
            ...
            MTNControlBase.SetValueInEditTable(_terminalConfigForm.tblGeneric,
                @"Allow Duplicate Prenote Ids for Cargo Types Allowing Dups", @"1", EditRowDataType.CheckBox, doReverse: true);
            _terminalConfigForm.DoSave();
        }


        [TestMethod]
        public void InboundDeleteProcessAllowDuplicatePrenoteTrue()
        {
            MTNInitialize();
           ...
           // FormObjectBase.NavigationMenuSelection(@"Gate Functions|Road Gate", forceReset: true);
            ...
            roadGateForm.txtNewItem.SetValue($"JLG{TestCaseNumber}", 10);
            ...
            MTNControlBase.SetValueInEditTable(cargoEnquiryForm.tblSearchCriteria, @"Site (Current)", Site.PreNotified,
                EditRowDataType.ComboBoxEdit, fromCargoEnquiry: true);
            cargoEnquiryForm.DoSearch();
            // MTNControlBase.FindClickRowInTable(cargoEnquiryForm.tblData, @"ID^" + CargoId + "~Voyage^MSCK000002~State^Prenotified");
           cargoEnquiryForm.tblData2.FindClickRow(["ID^" + CargoId + "~Voyage^MSCK000002~State^Prenotified"]);            cargoEnquiryForm.DoViewTransactions();
            ...
            _ediCAMProtocolForm.TblProtocols.FindClickRow(["Protocol Description^User Defined Event JRW-Report", "Name^Test43771"], xOffset: 40);

            _ediCAMProtocolForm.GetDetailsTabAndDetails();
            _ediCAMProtocolForm.GetScheduledTimesTabAndDetails(findTab: true);
            
            _ediCAMProtocolForm.txtStartDate.SetValue(loadFileDeleteStartTime.ToString(@"ddMMyyyy"), additionalWaitTimeout: 100);
            _ediCAMProtocolForm.txtStartTime.SetValue( loadFileDeleteStartTime.ToString(@"HHmm"), additionalWaitTimeout: 100);
            _ediCAMProtocolForm.txtEndDate.SetValue( endTS.ToString(@"ddMMyyyy"), additionalWaitTimeout: 100);
            _ediCAMProtocolForm.txtEndTime.SetValue( endTS.ToString(@"HHmm"), additionalWaitTimeout: 100);
            ...
            _ediCAMProtocolForm.TblProtocolTaskHistory.FindClickRow(["Task Status^Complete"], ClickType.DoubleClick);
           ...
           /* _ediProtocolTaskHistoryForm.SaveMessageAsFile(dataDirectory + TestCaseNumber + @"_CAMProtocolTaskHistory.txt");

           List<string> fileLines =
               _ediProtocolTaskHistoryForm.ReadMessageDetailsFromFile(
                   dataDirectory + TestCaseNumber + @"_CAMProtocolTaskHistory.txt");
           _ediProtocolTaskHistoryForm.CloseForm();

           CheckFileDetails(fileLines, @"TT1;Deleted;");
           CheckFileDetails(fileLines, @";;;" + CargoId + ";");*/
        ...
        /*private static void CheckFileDetails(List<string> fileLines, string detailToCheck)
        {
            ...
            {
                if (fileLines[index].Trim().Contains(detailToCheck))
                {
                    matchFound = true;
                    break;
                }
            ...
            Assert.IsTrue(matchFound,
                @"TestCase43771 - The following details where not found in the file:\n" + detailToCheck);
        }*/
