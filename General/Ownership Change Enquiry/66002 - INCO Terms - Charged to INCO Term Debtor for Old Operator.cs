...
namespace MTNAutomationTests.TestCases.Master_Terminal.General.Ownership_Change_Enquiry
{
    ...
    public class TestCase66002 : MTNBase
    {
        ...
        void MTNInitialize()
        {
            ...
            CallJadeScriptToRun(TestContext, "resetData_66002");
        }

        [TestMethod]
        public void INCOPastChargeOld()
        {
            MTNInitialize();
            ...
            ownershipChangeMaintForm
                .AddCOO(new OwnershipChangeDetailsAddArguments
                {
                    COOFieldsToSet = new []
                    {
                        ...
                        { FieldRowName = OwnershipChangeMaintForm.ControlFieldName.COONumber, FieldRowValue = $"{COOId}C" },
                        new MTNGeneralArguments.FieldRowNameValueArguments
                        ...
                        new MTNGeneralArguments.FieldRowNameValueArguments
                        { FieldRowName = OwnershipChangeMaintForm.ControlFieldName.ChargeStorageToINCOTerms, FieldRowValue = "INCOT2\tINCOT2 Description" },
                    }, 
            ...
            cargoEnquiryTransactionForm.TblTransactions2.FindClickRow(new FindClickRowArguments
                { DetailsToSearchFor = new [] { "Type^Edited" } });
            cargoEnquiryTransactionForm.GetChargesTab();   // Currently have to do it this way as we use the tabcontrol to get the tab
            cargoEnquiryTransactionForm.INCODebtorChargedValidation(effective,
                new[] { $"Debtor^{MSLDebtor}~Transaction^Edited~Narration^ILTINCOT",
                    $"Debtor^{MSLDebtor}~Transaction^Edited~Narration^ILTINCOT" }, new [] { MSLDebtor, MSLDebtor});
